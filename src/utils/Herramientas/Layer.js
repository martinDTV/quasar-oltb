...
import {Keys} from 'oltb/src/oltb/js/helpers/constants/Keys'
import {Toast} from 'oltb/src/oltb/js/common/Toast';
import {Dialog} from 'oltb/src/oltb/js/common/Dialog'
import {Events} from 'oltb/src/oltb/js/helpers/constants/Events';
import { Control } from 'ol/control';
import {download} from 'oltb/src/oltb/js/helpers/browser/Download';
import {LogManager} from 'oltb/src/oltb/js/managers/LogManager';
import {LayerModal} from 'oltb/src/oltb/js/tools/modal-extensions/LayerModal';
...
import {instantiateLayer} from 'oltb/src/oltb/js/ol-types/LayerType';
import {createUICheckbox} from 'oltb/src/oltb/js/creators/CreateUICheckbox';
import {LocalStorageKeys} from 'oltb/src/oltb/js/helpers/constants/LocalStorageKeys';
import {SvgPaths, getIcon} from 'oltb/src/oltb/js/icons/GetIcon';
import {instantiateSource} from 'oltb/src/oltb/js/ol-types/SourceType';
import {instantiateFormat} from 'oltb/src/oltb/js/ol-types/FormatType';
...
import {DownloadLayerModal} from 'oltb/src/oltb/js/tools/modal-extensions/DownloadLayerModal';
import '../../css/SelectLayer.scss'
import '../../css/toolbox.scss'
import GetLayers from 'src/layers/Geonode';
...
const options = ['eje1','eje2','eje3','eje4']
...
class LayerTool extends Control {
    ...
    initToolboxHTML() {
        ...
        const html = (`
            <div id="${ID_PREFIX}-toolbox" class="${CLASS_TOOLBOX_SECTION}">
                <div class="${CLASS_TOOLBOX_SECTION}__header oltb-toggleable" data-oltb-toggleable-target="${ID_PREFIX}-map-toolbox-collapsed">
                    <h4 class="${CLASS_TOOLBOX_SECTION}__title" data-oltb-i18n="${I18N_BASE}.toolbox.titles.mapLayers">Mapas Layers</h4>
                ...
                <select id="selectEje" class="oltb-input">
                 <option selected disabled>Elija una opcion</option>
                 <option value="2">Eje 1</option>
                 <option value="3">Eje 2</option>
                </select>
                </div>
          ...
          `
                    ...
                    <div class="${CLASS_TOOLBOX_SECTION}__group ${this.options.disableMapCreateLayerButton ? `${CLASS_TOOLBOX_SECTION}__group--topmost` : ''}">
                        ${
                          `<ul id="${ID_PREFIX}-map-stack" className="${CLASS_TOOLBOX_LIST}"></ul>`
                        } <!-- Aqui es donde se tiene que mover para las secciones  -->
                    </div>
                </div>
      ...
      selecElement.addEventListener('change', () => {
        ...
        const selectedOption = selecElement.value;
        const apiurl = `https://geonode.appsmty.gob.mx/api/v2/datasets?page=${selectedOption}`
        const MapsLayers = LayerManager.getMapLayers();
        console.log(MapsLayers)
        GetLayers(apiurl, MapsLayers)
      })
    }
